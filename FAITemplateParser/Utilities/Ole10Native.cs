...
namespace FAITemplateParser.Utilities
{
    ...
    internal class Ole10Native
    {
        ...
        public static bool ExtractFile(string sourceFilePath, string destinationFolder)
        {
            bool resultboolean = false;
            int result;
            string originalfilename = System.IO.Path.GetFileName(sourceFilePath).Replace(System.IO.Path.GetExtension(sourceFilePath), "");
            // Open the structured storage file in READWRITE and SHARE_EXCLUSIVE
            result = StgOpenStorage(sourceFilePath, null, STGM.READWRITE | STGM.SHARE_EXCLUSIVE, IntPtr.Zero, 0, out IStorage iStorage1);

            uint numReturned;

            // Retrieve the array of STATSTG structures in the structured storage.
            iStorage1.EnumElements(0, IntPtr.Zero, 0, out IEnumSTATSTG pEnumStatStg);
            ...
            do
            {
                ...
                if (numReturned != 0)
                {
                    ...
                    if (ss[0].pwcsName.Contains("Ole10Native") == true)
                    {
                        ...
                        System.IO.BinaryWriter bWriter = new System.IO.BinaryWriter(System.IO.File.Open(destinationFolder + "\\" + "{" + originalfilename + "}" + fileName, System.IO.FileMode.Create));
